...
const configFile = path.resolve(process.env.DOORMAN_CONFIG || [configDir, "app.js"].join("/"));
...
function attributeError(file, attribute) {
  ...
  throw new Error(`Missing attribute '${attribute}' in file ${file}`);
}
...
function addDefaultOptions(c) {
  let newConf = c;
  for (let k in config) {
    if (Object.prototype.hasOwnProperty.call(config, k) && k != "domains") {
